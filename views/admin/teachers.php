...
<body>
    ...
    <main class="bg-gray-100 pt-24 pb-12 px-5">
        ...
        <section class="flex flex-wrap items-center justify-between gap-5 py-5">
            <h1 class="text-3xl text-gray-800 font-bold">Les Enseignants</h1>
        ...
        <section class="p-8 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <?php
                $teachers = $administrator->showUsers(3);
                if ($teachers) {
                    foreach ($teachers as $teacher) {
                        $enseignant = new Teacher($teacher['id_user'],$teacher['nom'],$teacher['prenom'],$teacher['phone'],$teacher['email'],$teacher['password'],'Enseignant',$teacher['statut'],$teacher['photo']);
            ?>
                ...
                <div class="bg-white rounded-xl shadow-lg hover:shadow-xl p-6 animate__animated animate__fadeIn hover:shadow-lg transition-shadow duration-300">
                    <div class="flex items-center justify-center">
                        <div class="flex flex-col items-center gap-2">
                            <img src="../../uploads/<?php echo $enseignant->getPhoto() ?>" alt="Profile" class="h-16 w-16 rounded-full object-cover mb-4">
                            <p class="text-lg font-semibold text-gray-800"><?php echo $enseignant->getPrenom().' '.$enseignant->getNom() ?></p>
                            <a href="mailto: <?php echo $enseignant->getEmail() ?>"><p class="text-sm text-gray-600"><?php echo $enseignant->getEmail() ?></p></a>
                            <a href="tel: <?php echo $enseignant->getTelephone() ?>"><p class="text-sm text-gray-600"><?php echo $enseignant->getTelephone() ?></p></a>

                            <?php
                                if($enseignant->getStatus() == 'Actif') {
                            ?>
                            <form method="POST">
                                <input type="hidden" name="user" value="<?php echo $enseignant->getId() ?>">
                                <button name="suspend" class="duration-300 text-white bg-red-600 py-1 px-4 rounded-sm text-xs hover:bg-red-500">Suspendre <i class="ml-1 fa-solid fa-ban"></i></button>
                            </form>
                            <?php
                                }else{
                            ?>
